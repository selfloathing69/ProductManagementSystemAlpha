    ...
    public class ProductLogic
    {
        ...
        private void InitializeDataIfEmpty()
        {
            try
            {
                ...
                {
                    // Обновляем счётчик на основе максимального номера в базе
                    var maxNumber = existingProducts.Max(p => p.Number);
                    _nextNumber = maxNumber + 1;
                }
            }
            catch
            {
        ...
        /// Присваивает внутренний номер товару, ID будет присвоен базой данных.
        /// </summary>
        /// <param name="product">Товар для добавления</param>
        /// <returns>Добавленный товар с присвоенным номером и ID</returns>
        public Product AddProduct(Product product)
        {
            ...
            if (_repository != null)
            {
                _repository.Add(product);  // Добавляем товар в базу данных, ID присваивается автоматически
            }
            else
            {
                _products.Add(product);  // Добавляем товар в локальный список
            }
